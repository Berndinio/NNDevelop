...
# out = pickle.load(open("model_saves/v9/losses.pkl", "rb"))
# print(out)
...
train_loader = DataLoader(
    dataset=AmazonDataset("data/Cell_Phones_and_Accessories_5_preprocessed.h5", dataset_scaling, 0),
    batch_size=batch_size, shuffle=True, num_workers=8)
print("Finished Loading Training Data...")
test_loader = DataLoader(
    dataset=AmazonDataset("data/Cell_Phones_and_Accessories_5_preprocessed.h5", dataset_scaling, 1),
    batch_size=batch_size, shuffle=False, num_workers=8)
print("Finished Loading Testing Data...")
valid_loader = DataLoader(
    dataset=AmazonDataset("data/Cell_Phones_and_Accessories_5_preprocessed.h5", dataset_scaling, 2),
    batch_size=batch_size, shuffle=False, num_workers=8)
...
model.classifier = nn.Linear(model.config.hidden_size, num_labels).to(Constants.device)
...
print("Validation dataset length: ", len(valid_loader))
...
for epoch in range(num_epochs):
    ...
    all_losses[0].append(running_loss / (len(train_loader) * batch_size))
    ...
    for i, (target, net_input) in enumerate(test_loader):
        ...
        # print statistics
        running_loss += loss.item()
    all_losses[1].append(running_loss / (len(train_loader) * batch_size))
    pickle.dump(all_losses, open("model_saves/v" + str(next_save_path) + "/losses.pkl", 'wb'))
    print("\n")
    # noinspection PyUnboundLocalVariable
    print('[%d, %5d] Test loss: %.3f' % (epoch + 1, i + 1, running_loss / len(test_loader)))
    torch.save(model.state_dict(),
               "model_saves/v" + str(next_save_path) + "/epoch_" + str(epoch) + "-loss_" + str(
                   running_loss / len(test_loader)) + ".pt")
